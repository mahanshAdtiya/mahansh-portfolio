...
export function Projects() {
  return (
    <section
      id="projects"
      className="mx-auto max-w-page px-gutter pb-[clamp(64px,14vh,150px)]"
    >
      <SectionHeading
        num="05"
        ...
        {PROJECTS.map((project) => (
          <li key={project.title}>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={`group relative block h-full border border-edge py-[22px] pl-[22px] pr-11 transition-[transform,border-color,box-shadow] duration-[420ms] ${EASE} hover:-translate-y-1.5 hover:border-heading hover:shadow-lift`}
            >
              <span
                aria-hidden="true"
                className={`absolute top-4 right-4 -translate-x-[5px] translate-y-[5px] text-accent opacity-0 transition-[opacity,transform] duration-[320ms] ${EASE} group-hover:translate-x-0 group-hover:translate-y-0 group-hover:opacity-100 group-focus-visible:translate-x-0 group-focus-visible:translate-y-0 group-focus-visible:opacity-100`}
              >
                <svg
                  viewBox="0 0 24 24"
                  width="18"
